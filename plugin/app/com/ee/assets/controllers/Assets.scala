...
class AssetsBuilder extends Controller {
  ...
  def at(path: String, file: String): Action[AnyContent] = Action { request =>
    ...
    resourceNameAt(path, file).map { resourceName =>
       ...
       def loadResource(name:String) : Option[java.net.URL] = {
      ...
      val acceptGzip = request.headers.get(ACCEPT_ENCODING).map(_.split(',').exists(_.trim == "gzip")).getOrElse(false)

      val resource = if(resourceName.contains(".gz.") && acceptGzip){
        loadResource(resourceName).map(_ -> true)
      } else {
        loadResource(resourceName).map(_ -> false)
      }
